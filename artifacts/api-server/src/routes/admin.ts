...
// ──────────────────────────────────────────────────────────
// LEADER CONFIG — frontend account / tenant switcher
// ──────────────────────────────────────────────────────────
const TENANT_CONFIGS_ADMIN: Record<string, Record<string, string>> = {
  nirmal: {
...
router.get("/admin/leader-config/tenants", requireRole("super_admin"), async (_req, res) => {
  try {
    const row = await db.select({ value: siteConfigTable.value })
      .from(siteConfigTable)
      .where(eq(siteConfigTable.key, "leader_config"))
      .limit(1);
    const current: Record<string, string> | null = row[0]?.value
      ? (JSON.parse(row[0].value) as Record<string, string>)
      : null;
    const activeKey = current
      ? (Object.keys(TENANT_CONFIGS_ADMIN).find(
          k => TENANT_CONFIGS_ADMIN[k].nameEn === current.nameEn,
        ) ?? null)
      : null;
    const tenants = Object.entries(TENANT_CONFIGS_ADMIN).map(([key, cfg]) => ({
      key,
      nameEn: cfg.nameEn,
      nameTa: cfg.nameTa,
      constituencyEn: cfg.constituencyEn,
      constituencyTa: cfg.constituencyTa,
      logoInitial: cfg.logoInitial,
      siteTitle: cfg.siteTitle,
      email: cfg.email,
    }));
    res.json({ tenants, activeKey });
  } catch (err) {
    console.error("[admin] leader-config tenants:", err);
    res.status(500).json({ error: "Internal server error" });
  }
});

router.post("/admin/leader-config/activate", requireRole("super_admin"), async (req: AuthRequest, res) => {
  try {
    const { tenant } = req.body as { tenant?: string };
    if (!tenant || !TENANT_CONFIGS_ADMIN[tenant]) {
      res.status(400).json({ error: "Unknown tenant key" });
      return;
    }
    const value = JSON.stringify(TENANT_CONFIGS_ADMIN[tenant]);
    const existing = await db.select({ id: siteConfigTable.id })
      .from(siteConfigTable)
      .where(eq(siteConfigTable.key, "leader_config"))
      .limit(1);
    if (existing.length > 0) {
      await db.update(siteConfigTable)
        .set({ value })
        .where(eq(siteConfigTable.key, "leader_config"));
    } else {
      await db.insert(siteConfigTable).values({ key: "leader_config", value });
    }
    await logAudit(req, "UPDATE", `leader_config:${tenant}`);
    res.json({ ok: true, tenant });
  } catch (err) {
    console.error("[admin] leader-config activate:", err);
    res.status(500).json({ error: "Internal server error" });
  }
});
